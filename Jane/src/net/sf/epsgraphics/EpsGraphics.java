 ...
 * 
 * Modified by Andrew Michaud to use a StringBuffer to store data.
 */
...
public class EpsGraphics extends java.awt.Graphics2D {
     ...
     */
    public EpsGraphics(
        String title, int minX, int minY, int maxX, int maxY,
        ColorMode colorMode) {
    ...
    public void drawRenderedImage(RenderedImage img, AffineTransform xform) {
        ...
        BufferedImage img1 = new BufferedImage(
                                cm, wr, cm.isAlphaPremultiplied(), properties);
    ...
    public void drawString(String s, float x, float y) {
        ...
        append(newX + " " + newY + " moveto");
        append("gsave\n");
        String font = _font.getName();
        int size = _font.getSize();
        append("/" + font + " " + size + " selectfont");
        append("( " + s + " ) show");
        append("grestore\n");
    }
    ...
    public void setStroke(Stroke s) {
        if (s instanceof BasicStroke) {
            ...
            if (lineWidth != _stroke.getLineWidth()) {
                append(_stroke.getLineWidth() + " setlinewidth");
                lineWidth = _stroke.getLineWidth();
            }
            
            if (miterlimit != _stroke.getMiterLimit()) {
                double miterLimit = _stroke.getMiterLimit();
                if (miterLimit < 1.0f)
                    miterLimit = 1;

                append(miterLimit + " setmiterlimit");
                this.miterlimit = _stroke.getMiterLimit();
            }
            
            if (linejoin != _stroke.getLineJoin()) {
                append(_stroke.getLineJoin() + " setlinejoin");
                this.linejoin = _stroke.getLineJoin();
            }
            
            if (linecap != _stroke.getEndCap()) {
                append(_stroke.getEndCap() + " setlinecap");
                this.linecap = _stroke.getEndCap();
            }
    ...
    public void setColor(Color color) {
        if (color == null)
            ...
            // no reason to "change" the color if it's already what the 
            // user wants.              
        } else {
            this.color = color;
            switch (colorMode) {
            case BLACK_AND_WHITE:
                ...
                break;
            case GRAYSCALE:
                double grayvalue = ((color.getRed() + color.getGreen()
                                        + color.getBlue()) / (3 * 255f));
                append(grayvalue + " setgray");
                break;
            case COLOR_RGB:
                append((color.getRed() / 255f) + " "
                            + (color.getGreen() / 255f) + " "
     ...
     */
    public boolean drawImage(
        Image img, int x, int y, int width, int height, Color bgcolor,
        ImageObserver observer) {

        return drawImage(
            img, x, y, x + width, y + height, 0, 0, width, height,
            bgcolor, observer);
    }
    ...
    public boolean drawImage(
        ...
        return drawImage(
            img, dx1, dy1, dx2, dy2, sx1, sy1, sx2, sy2,
    ...
    public boolean drawImage(
        ...
        if (this.colorMode.equals(ColorMode.BLACK_AND_WHITE)
            ...
            append("{currentfile " + width
                        + " string readhexstring pop} bind");
            append("image");
        } else {
            // no difference between RGB and CMYK
                    ...
                    line.append(
                        toHexString(
                            (color.getRed() + color.getGreen()
                                + color.getBlue()) / 3));
                } else { // No difference between RGB and CMYK
    ...
    public void addComment(String comment) {
        _document.append(this, comment);
    }

    public int getMaxX() {
        return _document.getMaxX();
    }
    ...
    public String toString() {
        ...
        return _document.getContents().toString();
    }
